...
// PATH 刻意只含系统目录（win32=System32，posix=/usr/bin:/bin）——零配置机器的等价环境
// （posix 链路实际依赖：/bin/sh 解释 pnpm shim，内核按 shebang 绝对路径解析不经 PATH）。
// 前置：系统 node ≥22.19（冒烟用系统 node 充当 execPath）+ 同级 deepseek-harness 检出。
const [nodeMajor, nodeMinor] = process.version.slice(1).split('.').map(Number)
const nodeOk = (nodeMajor === 22 && nodeMinor >= 19) || nodeMajor >= 24
const harnessReady = existsSync(join(__dirname, '..', '..', '..', '..', 'deepseek-harness', 'apps', 'cli', 'src', 'bin.ts'))
...
describe.skipIf(process.env.DSH_DESKTOP_PLUGIN_SMOKE !== '1' || !nodeOk || !harnessReady)('market seed smoke', () => {
  it('seeds a dshmarket package via the dsh CLI under a clean PATH, then applies the restart guard', { timeout: 180_000 }, async () => {
    ...
    const env = buildSidecarEnv(paths, { PATH: process.platform === 'win32' ? 'C:\\Windows\\System32' : '/usr/bin:/bin' }, { shimDir })
    const lines: string[] = []
