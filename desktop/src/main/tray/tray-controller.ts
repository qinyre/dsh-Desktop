...
/** 托盘（设计书 §5）：显示 / 插件市场 / 重启服务 / 打开日志目录 / 退出。 */
export class TrayController {
  private tray: Tray | undefined
  private trayHintShown = false
  private quiting = false

  constructor(private readonly opts: { iconPath: string; logDir: string; onShow(): void; onMarket(): void; onRestart(): void; onQuit(): void }) {}
  ...
  attach(win: BrowserWindow): void {
    ...
    this.tray.setContextMenu(Menu.buildFromTemplate([
      ...
      // 插件市场在 Web UI 的设置页里（dshmarket）；重启服务承接市场安装后的待重启提示。
      { label: '插件市场', click: () => this.opts.onMarket() },
